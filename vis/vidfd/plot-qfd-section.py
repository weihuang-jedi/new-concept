...
class GeneratePlot():
  ...
  def setup_default(self):
   ...
   #self.cblevs = np.arange(-200.0, 220.0, 20.0)
...
class PlotVariable():
 ...
 #-----------------------------------------------------------------------------------------
  def get_data(self, year=2021, month=12):
    ...
    varname = 'div'
    fullname = '%s/monthly_mean_gfs_QFDIV_%s.nc' %(dirname, mon_year)
   ...
   #self.missing_value = var.missing_value
    ...
    for t in ['00', '06', '12', '18']:
      fullname = '%s/monthly_mean_gfs_dfdiv_%sZ_%s.nc' %(dirname, t, mon_year)
      ncf = nc4.Dataset(fullname, 'r')
 ...
 #-----------------------------------------------------------------------------------------
  def process(self, datadir=None):
   ...
   #val = meanval[:, :, nlon-length:nlon+length]
    ...
    zp = z[nz:]
    vp = 100.0*pvar[nz:, :]
    title = 'GFS Dec 2021 Density Flux %s' %(longname)
