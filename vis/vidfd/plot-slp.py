...
class PlotVariable():
 ...
 #-----------------------------------------------------------------------------------------
  def process(self, flnm=None, imgname='ERA5 VIDFD', it=0):
   ...
   #time_convert = nc4.num2date(self.time, timevar.units, timevar.calendar)
   #print('time_convert = ', time_convert)
   #print('self.time = ', self.time)
   #print('sincetime = ', sincetime)

   #month_index = self.get_index(year=2021, month=12, hour=0, self.time)
    mslvar = ncf.variables['msl']

    meanval, mean_00, mean_06, mean_12, mean_18 = self.get_mean(mslvar, year=2021, month=12)
    longname = mslvar.getncattr('long_name')
   #nt = 0
   #for it in month_index:
   #  val = mslvar[it,:,:]
   #  if(0 == nt):
   #    sum = val
   #  else:
   ...
   #  nt += 1
   #mslval = sum/nt
   #print('mslval min: %f, max: %f' %(np.min(mslval), np.max(mslval)))
