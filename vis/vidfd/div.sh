...
monthly_mean_gfs_div_${hour}Z_dec_2021.nc
monthly_mean_gfs_div_${hour}Z_dec_2021
monthly mean GFS DIV ${hour}Z Dec 2021
EOF
   sed -e "s/DATAINFO/datainfo_${hour}.txt/g" \
       calculate-divergence.template > calculate-divergence-${hour}.ncl
   ncl calculate-divergence-${hour}.ncl &
 done

 sed -e "s/DATAINFO/datainfo.txt/g" \
     calculate-divergence.template > calculate-divergence.ncl
 ncl calculate-divergence.ncl &
