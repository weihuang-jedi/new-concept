...
if __name__== '__main__':
 ...
 #datafile = '%s/hl_monthly_mean_gfs_4_202201_000.nc' %(datadir)
  file1 = '%s/hl_gfs_4_20220116_0000_000.nc' %(datadir)
  file2 = '%s/hl_gfs_4_20220116_0600_000.nc' %(datadir)
...
#-----------------------------------------------------------------------------------------
  ...
  ncf1 = nc4.Dataset(file1, 'r')
  ncf2 = nc4.Dataset(file2, 'r')
...
#-----------------------------------------------------------------------------------------
  grav = 9.81
  rgas = 287.05
  hgt = ncf1.variables['alt'][:]

  t1 = ncf1.variables['T'][:, :, :]
  p1 = ncf1.variables['P'][:, :, :]
  r1 = p1/(rgas*t1)

  nalt, nlat, nlon = t1.shape
 ...
 #----------------------------------------------------------------------------------------
  ...
  dtdp = dt*dp
 #----------------------------------------------------------------------------------------
 #Case 1: dt.drho >= 0, dp >=0, Thermal-Dynamic Warm High Dense
  opt1 = np.where(dtdrho >= 0.0, 1, 0)
  opt2 = np.where(dp >= 0.0, 1, 0)
  cate = np.where(opt1*opt2, 1, cate)

 #----------------------------------------------------------------------------------------
 #Case 2: dt.drho >= 0, dp < 0, Thermal-Dynamic Cold Low thin
  opt2 = np.where(dp < 0.0, 1, 0)
  cate = np.where(opt1*opt2, 2, cate)

 #----------------------------------------------------------------------------------------
 #Case 3: dt.drho < 0, dt.dp >= 0, dp > 0  Thermal Warm High Thin
 ...
 #----------------------------------------------------------------------------------------
 #Case 4: dt.drho < 0, dt.dp >= 0, dp < 0 Thermal Cold Low Dense
  opt3 = np.where(dp < 0.0, 1, 0)
  cate = np.where(opt1*opt2*opt3, 4, cate)

 #----------------------------------------------------------------------------------------
 #Case 5: dt.drho < 0, dt.dp < 0, dp >= 0 Dynamic Cold High Dense
  opt2 = np.where(dt*dp < 0.0, 1, 0)
  opt3 = np.where(dp >= 0.0, 1, 0)
  cate = np.where(opt1*opt2*opt3, 5, cate)

 #----------------------------------------------------------------------------------------
 #Case 6: dt.drho < 0, dt.dp < 0, dp < 0 Dynamic Warm Low Thin
  opt3 = np.where(dp < 0.0, 1, 0)
